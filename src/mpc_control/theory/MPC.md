...
# MPC 与 强化学习
以下是 MPC 问题喜闻乐见的**离散**形式，
$$
\begin{aligned}
\min_{U} \sum_{i=0}^{N} & l_i(x_i, u_i) + l_f(x_f) \tag{1} \\
...
\end{aligned}
$$
其中，$l_i$ 为 stage cost（**阶段代价**），即每个时刻的**代价值**；$l_f$ 为终止时刻的 stage cost；$x_{i+1} = f(x_i, u_i)$ 为**系统动力学方程**，也称**状态转移方程**；$g_1(X, U) = 0$ 为**等式约束**；$g_2(X, U) \ge 0$ 为不等式约束。
有意思的是，强化学习中的**状态价值函数** $V(x_i)$、**动作价值函数** $Q(x_i, u_i)$ 的概念在这里也是通用的，有，
$$
\begin{aligned}
V(x_i) &= \min_{u_i} \{ l(x_i, u_i) + V(x_{i+1}) \} \\
Q(x_i, u_i) &= l(x_i, u_i) + V(x_{i+1}) \tag{2} \\
V(x_i) &= \min_{u_i} \{ Q(x_i, u_i) \}
\end{aligned}
$$
动作-价值函数 $Q(x_i, u_i)$ 表示系统从状态 $x_i$ 出发，在当前时刻执行动作 $u_i$，之后都采取最优动作，得到动作 $u_i$ 前提下的“最优回报”，在这里是 i 时刻的 stage cost 加上 i+1 时刻的状态价值函数 $V(x_{i+1})$；状态价值函数 $V(x_{i+1})$；在最优控制中也称为 cost to go，表示系统处于状态 $x_i$ 时的最优价值，即当前时刻和之后的时刻都采取最优动作，最终得到的回报。  
在强化学习中，上述描述中的“**最优回报**”要改成“期望回报”，笔者的理解是当 policy 收敛之后，期望的也就是最优了（所以最优控制相当于基于系统收敛的前提来看待这个问题？）
对于强化学习，reward is enough，但是在MPC中，reward/cost is not enough，还需要知道系统的gradient，MPC与RL最大的区别也在于此，model-based的这个model可以指零阶或一阶，甚至二阶微分模型，有了gradient信息，系统就能够知道怎么往最优方向走，所以MPC相比于RL，能够在非常短的时间内收敛到最优值（RL的训练过程就是在优化迭代，同样的问题下，RL的policy收敛比MPC计算收敛要花多很多时间）。

### 另一方面可以思考的是，咱们MPC设计cost、约束，能不能像RL设计reward那么自由？
首先给出答案：可以！
RL的reward设计之所以可以那么随意，在我看来是因为不用人为设计reward对obs的微分，就算你给一些离散的、稀疏的reward设计，一样可以learn出来。但是RL真的不存在所谓的model和所谓的gradient吗，在我看来也不完全是这样，首先，RL训练需要一个仿真器，实际上也就是一个前向动力学模型（只不过仿真器的接触和碰撞动力学做的非常好），和MPC中的Rollout是一样的道理。另一方面，RL的模块化真的做的非常好，好到类似PPO之类的网络参数更新算法可以直接copy，让人忽略了RL也有gradient！实际上，RL的gradient在我看来与数值微分是类似的道理。感兴趣的朋友可以研究下deepmind开源的mujoco mpc项目，其中的几个需要梯度的方法原理上就是仿真器作为model，数值微分做gradient，相比RL少了一个agent，这也是我认为这个方法上限不高的原因之一。
所以如果我们想在MPC中随意地设计cost，那么可以考虑使用数值微分，但是有可能实时性就不能保证了，这个方法运算量是很大的（后面会说原因）。由于有gradient的存在，MPC才能更快地收敛到最优，但同时gradient通常是昂贵的，并且假设模型也通常是不准确的，这也导致部分MPC问题设计起来存在困难，而且鲁棒性没有目前的RL控制器那么强。MPC成也gradient！败也gradient！
